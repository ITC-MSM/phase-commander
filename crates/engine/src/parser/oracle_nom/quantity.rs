...
pub fn parse_quantity_ref(input: &str) -> OracleResult<'_, QuantityRef> {
    alt((
        ...
        // These specific "the number of" forms must precede the generic
        // `parse_the_number_of` arm. Keep them nested so the outer nom `alt`
        // remains below its 21-item tuple limit.
        alt((
            // CR 701.22a: the scry-context "number of cards looked at …"
            // reading must win over a plain object-count reading.
            parse_scry_look_count_ref,
            // CR 702.33d: kicker counts retain whether Oracle names this
            // object or the triggering spell.
