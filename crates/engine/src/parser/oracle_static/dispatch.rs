...
pub(crate) fn parse_static_line_inner(
    ...
    if super::shared::contains_activated_abilities_cant_be_activated(tp.lower) {
        ...
        if let Some(condition) = parse_unless_static_condition(&tp) {
            // CR 602.5: activating an ability is not an enforcement point that
            // runs the CR 118.12a payment round-trip, so an `UnlessPay` gate here
            // is deferred rather than accepted (see `attach_gated_condition`).
