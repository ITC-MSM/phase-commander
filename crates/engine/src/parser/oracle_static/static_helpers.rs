...
pub(crate) fn extract_cant_untap_condition(lower: &str) -> Option<StaticCondition> {
    ...
    //
    // Engine limitation (not CR-mandated — this is a runtime binding gap, not
    // a rules restriction): layer evaluation (`game::layers::evaluate_condition`)
    ...
    if let Some(unless_text) = nom_tag_lower(remaining, remaining, "unless ") {
        return match nom_condition::parse_unless_condition(unless_text) {
            Ok((rest, condition))
                if rest.trim().is_empty() && !condition.has_unbindable_designation_anchor() =>
            {
                Some(condition)
            }
